\section{Microscope near fume cupboard\label{sec:micro}}
  \begin{itemize}
  	...
  	\item To take photo, turn on camera and \texttt{press the LV button}. \texttt{Rotate dial next to AEK-AFL button} to adjust the acquisition time of the camera
	  	\item \cmd{Activate by pressing button on block with \quote{slider} image};
	\item \red{Work with green light when using optical resist};
	\item In RHUL clean room next to SEM login with \textbf{UserName}: olympus-user, \textbf{Password}:olympus1 and \textbf{THIS COMPUTER \red{Not CC}} \ira \quote{Motic Images} \ira \cmd{plug camera in} \ira \quote{Capture}  	
  \end{itemize}
